...
def print_detail(cs, i, mode, stringio)
  ...
  if i.op_count > 0 then
    ...
    i.operands.each_with_index do |op,c|
      if op.reg?
        ...
        stringio.puts "\t\toperands[#{c}].type: FP = 0x#{(uint32(op.value))}"
      elsif op.mem?
        ...
        if op.value[:disp].nonzero?
          stringio.puts "\t\t\toperands[#{c}].mem.disp: 0x%x" % (uint64(op.value[:disp]))
        end
      end
    end
  end
...
@platforms.each do |p|
  ...
  res.each do |i|
    ...
    print_detail(cs, i, cs.mode, ours)
    cache = i
  end
