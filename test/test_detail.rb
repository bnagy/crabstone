...
def print_detail(insn)
	if insn.regs_read.any?
		...
		puts "\t\tImplicit registers read: #{read}"
	end

	if insn.regs_write.any?
		written = insn.regs_write.map {|r| insn.reg_name r}.join(' ')
		puts "\t\tImplicit registers modified: #{written}"
	end

	if insn.groups.any?
		puts "\t\tThis instruction belongs to groups: #{insn.groups.join(' ')}"
	end
end
...
def test_class()
	...
	@platforms.each do |p|
		...
		cs.disasm(p['code'], 0x1000) do |insn|
			print "\t0x#{insn.address.to_s(16)}: #{insn.mnemonic}\t#{insn.operand_string}"
			if insn.id.nonzero? then
				print "\t// insn-id: #{insn.id}, insn-name: #{insn.insn_name()}"
			end
			puts
			print_detail insn
		end
